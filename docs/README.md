...
**当前状态：文件骨架、数据字段与函数签名已建立，业务功能待实现。** 可以按暂定分工补充方法体；调用占位方法会抛 `NotImplementedError`，应用启动流程还不能运行。
...
3. 按 [README 的暂定分工](../README.md#小组成员排名不分先后)，在[功能清单](./功能列表.csv)筛选自己的名字；共同负责的条目先商量谁实现、谁配合，并写到开发备注。P0 是核心，P1 是配套，P2 是后续候选。
4. 从[系统设计的详细目录](./architecture.md#目录)按需跳转：先看第 1、2 节的分层与文件落位，再读第 3.1、3.2 节公共数据格式，最后找到自己模块的 Python 类型、SQL 和第 4 节接口。第 4.5 节有会员查询的方法体示例，第 6、7 节分别说明 CLI 与统一异常处理。
...
- 本轮业务讨论已收束，首版 84 项当前均为“待开发”；可选 TUI、冻结、候补、退款共 4 项 P2 候选保留“待确认”，不影响首版开发。
...
| 2. 对齐数据 | 先看设计第 3 节，再核对 `src/models/contracts.py` 的输入输出类型与会员表 | 姓名、联系方式、空值和返回类型只有一套约定，列表不临时改成字典 |
...
前提：已安装 Python 3.10 或以上版本；当前检查只用标准库，无需安装 MySQL 或 TUI 依赖。在仓库根目录（能看到 main.py 的目录）执行：

```bash
python -B -X utf8 -m unittest discover -s tests -p test_contract_consistency.py -v
```

此命令已验证可运行，预期显示 3 项检查和 `OK`，表示公共字段/默认值、服务与数据访问签名、SQL 列类型和可空性与设计一致。它不会连接数据库，也不代表业务可运行。若出现 `FAILED`，根据报错的类、方法或表名同时核对设计和源码，不要只删掉断言；若显示 `Ran 0 tests`，检查所在目录和文件名。若找不到 python，先安装 Python 并确认终端可执行该命令。
...
| 文件 | 用途 | 对应交付物 |
|------|------|--------------|
| [`功能列表.csv`](./功能列表.csv) | 功能范围、优先级、负责人、状态与验收要点 | Project Proposal / 开发与测试 |
...
设计细节集中在 `architecture.md`，参考上学期 OOP 的设计文档写法。第 3 节的数据字段与第 4 节的接口已落入骨架文件；有方法体的文档示例用于说明后续写法，源码当前仍仅有签名占位，SQL 文件只有注释。

默认 CLI，通过 `--tui` 进入可选 TUI；业务功能先在 CLI 完成与验收，TUI 根据时间补充。
